...
func (trans Sitrans) Event(msg *sipmess.SIPMessage) {
...
func (trans *Sitrans) handle_msg(msg *sipmess.SIPMessage) {
	...
	if msg.Request != nil {
		if msg.Request.Method == sipmess.Ack && trans.state == completed {
		...
		if msg.Request.Method == sipmess.Invite && trans.state == completed {
...
func call_core_callback(sitrans *Sitrans, message *sipmess.SIPMessage) {
...
func call_transport_callback(sitrans *Sitrans, message *sipmess.SIPMessage) {
...
func makeGenericResponse(status_code int, reason []byte, request *sipmess.SIPMessage) *sipmess.SIPMessage {
	req_hdr := request.Headers
	res_hdr := make(map[sipmess.SIPHeader][][]byte)

	// if value, ok := req_hdr[header.RecordRoute]; ok {
	// 	res_hdr[header.RecordRoute] = value
	// }

	if value, ok := req_hdr[sipmess.Via]; ok {
		res_hdr[sipmess.Via] = value
	}

	if value, ok := req_hdr[sipmess.SessionID]; ok {
		res_hdr[sipmess.SessionID] = value
	}

	res_hdr[sipmess.ContentLength] = [][]byte{[]byte("0")}

	return &sipmess.SIPMessage{
		Startline:  sipmess.Startline{Response: &sipmess.Response{StatusCode: status_code, ReasonPhrase: reason}},
		From:       request.From,
		To:         request.To,
